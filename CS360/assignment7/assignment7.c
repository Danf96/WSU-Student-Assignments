...
typedef struct _sortParams {
    ...
    int left;
    int right;
    int threads;	//tracks number of threads available
} SortParams;
...
static void quickSort(void* p) {
    ...
    int threads = params->threads; 	//threads, check, and thread added for multithreading purposes
    int check;
    pthread_t thread;
    int i = left, j = right;
    ...
    if (j - i > SORT_THRESHOLD) {           /* if the sort range is substantial, use quick sort */
        ...
        int firstT, secondT;
        if ((threads % 2) == 1){ 		//if odd number of threads is left, still do bit shifting for division but compensate for it
        	firstT = (threads - 1) >> 1;
        	secondT = (threads >> 1) + 1;
        }
        else{
        ...
        SortParams first;  first.array = array; first.left = left; first.right = j; first.threads = firstT; 
        char isThread = 0; 						//flag to ensure we're only waiting for threads that exist

        if (first.threads > 0 && (right - left > THREAD_THRESHOLD)){ 	//at a certain point, it's no longer worth making more threads as the overhead is too expensive
        	
        	isThread = 1;
        	first.threads--; //decrement because we area about to create a new thread
        	check = pthread_create(&thread, NULL, (void *) quickSort, (void *) &first);
        	if (check != 0){
			fprintf(stderr, "%s\n", strerror(check));
			exit(1);
		}
        ...
        second.threads = secondT; 
        quickSort(&second);                 /* sort the right partition */
        if(isThread == 1){ 			//only wait for thread if one was created
...
void sortThreaded(char** array, unsigned int count) {
    ...
    parameters.threads = maximumThreads; //parameters.thresh = 0;
